  ...
  return (
    <section id="home" className="relative min-h-screen flex items-center justify-center overflow-hidden">
      ...
      <div className="absolute inset-0 z-0">
        ...
        {/* Floodlight glows — static (no animation) + reduced blur radius for iOS/WebKit performance */}
        <div className="absolute top-[-10%] left-1/4 w-[600px] h-[400px] bg-apl-saffron/20 rounded-full blur-[60px]" />
        <div className="absolute top-[-5%] right-1/4 w-[500px] h-[300px] bg-apl-gold/25 rounded-full blur-[50px]" />
        <div className="absolute bottom-[10%] left-1/2 -translate-x-1/2 w-[800px] h-[300px] bg-apl-saffron-light/15 rounded-full blur-[70px]" />
        ...
        {/* Floating confetti particles — NOT rendered at all on mobile (not just hidden), so no animation loop runs */}
        ...
        {/* Floating cricket ball — desktop only */}
        {!isMobile && (
          <motion.div
            className="absolute right-[8%] top-[22%] w-20 h-20 rounded-full bg-gradient-to-br from-[#C0392B] to-[#7B241C] shadow-2xl border-2 border-[#922B21]"
            animate={{
              y: [0, -25, 0],
              rotate: [0, 360],
            }}
        ...
        {/* Floating bat icon shadow — desktop only */}
        {!isMobile && (
          <motion.div
            className="absolute left-[6%] bottom-[20%] w-16 h-16 rounded-full bg-apl-gold/20 blur-xl"
            animate={{
              y: [0, 20, 0],
              scale: [1, 1.1, 1],
            }}
